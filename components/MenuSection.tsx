...
import { MenuItem, MenuCategoryId } from "../types/menu";
...
export const MenuSection = ({
  ...
  const activeCategoryInfo = categories.find((category) => category.id === currentCategory) ?? categories[0];
    ...
    <section className="section menu-section" id="cardapio">
      <div className="section-heading">
        ...
        <p>
          {storeConfig.lojaAberta
            ? `Tempo estimado: ${storeConfig.tempoEstimadoEntrega}. Atualmente você está em: ${activeCategoryInfo?.name ?? "cardapio"}.`
      ...
      {!storeConfig.lojaAberta && (
        <div className="store-status-banner">
          ...
          <span>O cardapio continua visivel, mas a finalizacao fica bloqueada ate o admin abrir a loja.</span>
        </div>
      )}

      {isLoading ? (
        <div className="menu-state">Carregando cardapio...</div>
      ) : categories.length === 0 ? (
        <div className="menu-state">Nenhuma categoria ativa encontrada.</div>
      ) : (
        <>
          <CategoryTabs categories={categories} activeCategory={currentCategory} onChange={setActiveCategory} />
